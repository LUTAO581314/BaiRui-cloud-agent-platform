...
async function setup(options = {}) {
  ...
  const server = createPlatformServer({ repository, sessionSecret, secureCookies: false, agentIngestToken: "agent-ingest-test-token", licensePrivateKey: privateKey, providerVault: new SecretEnvelope(providerEncryptionKey), styles: "", logo: Buffer.from("logo"), icon: Buffer.from("icon"), loginScript: "login", adminScript: "admin-only", bailongmaUi, bailongmaOverlayCss: "overlay-css", bailongmaOverlayScript: "overlay-script", bairuiWorkspaceScript: "workspace-overlay", bairuiWorkspaceConversationsScript: "workspace-conversations-overlay", bairuiWorkspaceAgentsScript: "workspace-agents-overlay", bairuiWorkspaceChannelsScript: "workspace-channels-overlay", bairuiWorkspaceHotspotsScript: "workspace-hotspots-overlay", bairuiWorkspaceUsageScript: "workspace-usage-overlay", bairuiWorkspaceMemoryScript: "workspace-memory-overlay", bairuiWorkspaceSkillsScript: "workspace-skills-overlay", bairuiWorkspaceRunsScript: "workspace-runs-overlay", bairuiWorkspaceJobsScript: "workspace-jobs-overlay", bairuiWorkspaceHermesScript: "workspace-hermes-overlay", bairuiWorkspaceSettingsScript: "workspace-settings-overlay", bailongmaSceneBootstrap: "export function bootstrapScene() {}", logger: { error() {} }, ...options });
...
test("anonymous and ordinary users cannot access administrator data", async (t) => {
  ...
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-channels.js`, { headers: { cookie } })).text(), "workspace-channels-overlay");
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-hotspots.js`, { headers: { cookie } })).text(), "workspace-hotspots-overlay");
  ...
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-runs.js`, { headers: { cookie } })).text(), "workspace-runs-overlay");
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-jobs.js`, { headers: { cookie } })).text(), "workspace-jobs-overlay");
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-hermes.js`, { headers: { cookie } })).text(), "workspace-hermes-overlay");
  assert.equal(await (await fetch(`${context.baseUrl}/assets/bairui-workspace-settings.js`, { headers: { cookie } })).text(), "workspace-settings-overlay");
