...
test("resource collector reads only managed containers through fixed Docker arguments", async (t) => {
  ...
  fs.writeFileSync(path.join(instance, "instance.json"), JSON.stringify({ agentId: "agent_a", runtimeId: "runtime_a", deploymentId: "deployment_a", names: { hermes: "bairui-hermes-a", dashboard: "bairui-hermes-dashboard-a", runtime: "bairui-runtime-a" } }));
  const calls = [];
  const execFile = async (file, args) => {
    ...
    if (args[0] === "container" && args[1] === "ls") return { stdout: [
      ...
      JSON.stringify({ ID: "d".repeat(64), Names: "bairui-hermes-dashboard-a", Image: "hermes:test", State: "running" }),
    ...
    if (args[0] === "container" && args[1] === "inspect") return { stdout: JSON.stringify([
      ...
      { Id: "d".repeat(64), Name: "/bairui-hermes-dashboard-a", SizeRw: 512, State: { Status: "running", StartedAt: "2026-07-16T06:05:00.000Z" }, Config: { Image: "hermes:test", Labels: {} } },
    ...
    if (args[0] === "stats") return { stdout: [
      ...
      JSON.stringify({ Name: "bairui-hermes-dashboard-a", CPUPerc: "1.1%", MemUsage: "32MiB / 2GiB" }),
  ...
  assert.equal(samples[0].cpuPercent, 6.4);
  assert.equal(samples[0].memoryUsedBytes, 224 * 1024 * 1024);
  ...
  assert.equal(samples[0].containers[1].role, "hermes-dashboard");
  assert.equal(samples[0].containers[2].role, "runtime-boundary");
  assert.equal("version" in samples[0].containers[2], false);
  assert.equal("startedAt" in samples[0].containers[2], false);
