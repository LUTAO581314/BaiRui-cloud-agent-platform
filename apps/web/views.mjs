...
export function adminPage(principal) {
  ...
  <main class="control-main">
    <header class="control-topbar">
      ...
      <div class="control-top-actions">${platformAdmin ? `<label class="scope-select">组织<select id="organization-scope"><option value="">全部组织</option></select></label>` : ""}<span id="control-status" class="status unknown">正在读取</span><button id="refresh-admin" class="quiet-button" type="button">刷新</button></div>
    </header>
    ...
    <section class="admin-view" data-view-panel="config"><div class="section-toolbar"><div><h2>配置中心</h2><p>配置修订只记录声明状态，应用结果以 Supervisor 回执为准</p></div></div>${table("config-rows", ["修订", "Agent", "状态", "内容哈希", "创建者", "创建时间"])}</section>
    <section class="admin-view" data-view-panel="versions"><div class="section-toolbar"><div><h2>版本中心</h2><p>Hermes 和全部 upstream 的候选更新</p></div></div>${table("upstream-rows", ["项目", "当前版本", "候选版本", "兼容测试", "状态", "更新时间"])}<section class="control-section"><h2>平台发布</h2>${table("release-rows", ["版本", "状态", "Agent Commit", "创建时间"])}</section></section>
    ...
    <section class="admin-view" data-view-panel="release"><div class="section-toolbar"><div><h2>发布门禁</h2><p>GitHub CI、契约测试、迁移、备份和回滚条件</p></div><span class="executor-state">执行器状态由 Server Agent 上报</span></div>${table("release-gate-rows", ["门禁", "结果", "部署", "证据", "评估时间"])}</section>
    <section class="admin-view" data-view-panel="backups"><div class="section-toolbar"><div><h2>备份恢复</h2><p>PostgreSQL、Agent Workspace、记忆和配置备份记录</p></div><span class="executor-state">恢复操作需要已部署执行器</span></div>${table("backup-rows", ["类型", "状态", "部署", "大小", "加密", "验证时间", "到期时间"])}</section>
