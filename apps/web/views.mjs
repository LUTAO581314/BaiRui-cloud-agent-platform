...
export function adminPage(principal) {
  ...
  <main class="control-main">
    ...
    <section class="admin-view" data-view-panel="agents"><div class="section-toolbar"><div><h2>Agent 舰队</h2><p>每位用户的独立 Hermes Runtime 与服务器资源</p></div></div>${table("agent-rows", ["Agent", "所有者", "状态", "CPU", "内存", "主机存储", "版本", "运行环境", "启动时间", "操作"])}</section>
    ...
    <section class="admin-view" data-view-panel="integrations"><div class="section-toolbar"><div><h2>服务集成</h2><p>五层架构中的外部能力与最近执行结果</p></div><button id="refresh-hotspots" class="quiet-button" type="button">刷新热点</button></div>${table("integration-rows", ["项目", "所在层", "接入方式", "状态"])}<section class="control-section"><h2>Agent 授权状态</h2>${table("integration-authorization-rows", ["服务", "名称", "Agent", "用户", "状态", "凭证", "最近使用"])}</section><section class="control-section"><h2>最近运行</h2>${table("integration-run-rows", ["能力", "状态", "结果", "时间"])}</section></section>
    <section class="admin-view" data-view-panel="channels"><div class="section-toolbar"><div><h2>渠道舰队</h2><p>渠道绑定和 Adapter 回执</p></div></div>${table("channel-rows", ["渠道", "名称", "Agent", "用户", "状态", "能力", "Adapter", "最后健康", "最后收/发"])}</section>
    ...
    <section class="admin-view" data-view-panel="retention"><form id="retention-form" class="control-form retention-form"><h2>数据保留政策</h2><label>遥测（天）<input name="telemetryDays" type="number" min="1" max="3650" required></label><label>用量（天）<input name="usageDays" type="number" min="35" max="3650" required></label><label>审计（天）<input name="auditDays" type="number" min="1" max="3650" required></label><label>敏感访问事件（天）<input name="sensitiveAccessEventDays" type="number" min="1" max="3650" required></label><label>备份（天）<input name="backupDays" type="number" min="1" max="3650" required></label><div class="form-actions"><button class="primary-button compact" type="submit">保存政策</button><button id="run-retention" class="quiet-button" type="button">立即执行</button></div></form><section class="control-section"><h2>执行记录</h2>${table("retention-run-rows", ["开始时间", "状态", "清理数量", "备份过期命令", "完成时间", "错误"])}</section></section>
    ...
    <dialog id="agent-resource-dialog" class="control-dialog resource-dialog"><form method="dialog"><div class="section-heading"><div><h2 id="agent-resource-title">Agent 资源详情</h2><p id="agent-resource-subtitle"></p></div><button value="close" class="quiet-button" type="submit">关闭</button></div><div class="resource-summary"><div><span>状态</span><strong id="resource-detail-status">-</strong></div><div><span>CPU</span><strong id="resource-detail-cpu">-</strong></div><div><span>内存</span><strong id="resource-detail-memory">-</strong></div><div><span>Agent 存储</span><strong id="resource-detail-agent-storage">-</strong></div><div><span>主机存储</span><strong id="resource-detail-host-storage">-</strong></div><div><span>运行环境</span><strong id="resource-detail-environment">-</strong></div></div><section class="control-section"><h2>容器资源</h2>${table("resource-container-rows", ["角色", "状态", "容器", "CPU", "内存", "可写层", "版本 / 镜像", "启动时间"])}</section><section class="control-section"><h2>资源历史</h2>${table("resource-history-rows", ["采集时间", "状态", "CPU", "内存", "Agent 存储", "主机存储"])}</section></form></dialog>
  </main>
</div>`
  });
}
