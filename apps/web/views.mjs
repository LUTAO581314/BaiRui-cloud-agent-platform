...
export function adminPage(principal) {
  ...
  <main class="control-main">
    ...
    <section class="admin-view" data-view-panel="backups"><div class="section-toolbar"><div><h2>备份恢复</h2><p>PostgreSQL、Agent Workspace、记忆和配置备份记录</p></div><button id="create-agent-backup" class="primary-button compact" type="button">创建 Agent 备份</button></div>${table("backup-rows", ["类型", "状态", "部署", "大小", "加密", "验证时间", "到期时间", "操作"])}</section>
